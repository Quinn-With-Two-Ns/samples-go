...
type (
	...
	// CryptContext carries the active encryption keyID through workflow and
	// Go contexts. The DataConverter reads it via workflow.ContextAware to
	// pick the right key when encoding payloads.
	CryptContext struct {
		KeyID string `json:"keyId"`
	}
)

// PropagateKey is the key used to store CryptContext in the Context object.
...
// NewContextPropagator returns a context propagator that carries CryptContext
// across workflow boundaries via Temporal headers.
